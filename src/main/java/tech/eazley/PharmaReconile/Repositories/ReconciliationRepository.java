...
public interface ReconciliationRepository extends CrudRepository<Reconciliation,Integer> {
    ...
    @Query(
            nativeQuery = true,
            value = "select id, from_period as fromPeriod, to_Period as toPeriod, date_added as dateAdded, sagicor_totals as sagicorTotals, provider, charged, payable from pdfcache where pdfcache.pharmacy_id = ?1 and provider = ?2"
    )
    List<Reconciliation.PDFCacheProjection> findAllByPharmacyAndProvider(int pharmacyID, String provider);

    @Query(
            nativeQuery = true,
            value = "select id, from_period as fromPeriod, to_Period as toPeriod, date_added as dateAdded, sagicor_totals as sagicorTotals, provider, charged, payable from pdfcache where pdfcache.pharmacy_id = ?1"
    )
